document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('filter-btn')) {
            ...
            const projects = document.querySelectorAll('#github-projects > div.bento-box');
            
            projects.forEach(project => {
                if (filter === 'all') {
                    project.style.display = 'block';
                } else {
                    const lang = project.querySelector('.text-sm.bg-primary\\/20')?.textContent.toLowerCase();
                    if (lang && lang.includes(filter)) {
                        project.style.display = 'block';
                    } else {
    ...
    async function fetchGitHubProjects() {
        try {
            ...
            const apiUrl = `https://api.github.com/users/${username}/repos?sort=updated&per_page=${maxProjects}`;
                ...
                projectCard.innerHTML = `
                    ...
                    <div class="flex-grow">
                        ...
                        <div class="flex flex-wrap gap-3 mb-6">
                            ${project.language ? 
                              `<span class="text-sm px-3 py-1 bg-primary/20 text-primary rounded-full language-tag">${project.language}</span>` : ''}
